...
def test_escalation_stores_recovered_body(monkeypatch, tmp_path):
    ...
    idx = {}
    fr = fetch_mod._scrapling_fetch("https://portal.example/law", idx, lambda *_: None)
...
def test_escalation_returns_none_when_scrapling_fails(monkeypatch):
    ...
    assert fetch_mod._scrapling_fetch("https://portal.example/x", {}, lambda *_: None) is None
...
def test_escalation_respects_byte_cap(monkeypatch):
    ...
    assert fetch_mod._scrapling_fetch("https://portal.example/huge.pdf", {}, lambda *_: None) is None

...
SECTION_RE = re.compile(
    ...
    r"|§\s*\d+[A-Za-z]{0,2}"
    r"|\d{1,4}[A-Za-z]{0,2}\.\s*[‐-―]\s*\(\d"          #   SG "199.—(1)" section body start
    r"|\d+[A-Za-z]{0,2}\.\s+(?=[A-Z])"                           #   "26.  Foo" bare numbered clause
    r")"
    r")"
)

# Singapore SSO PDFs open with an "Arrangement of Provisions" table — entries like
# "199. Accounting records and systems of control" with no dotted leaders, so they slip past
# the generic TOC filter and the bare-number rule turns each into a bogus provision whose
# "body" is the next TOC entries. The real section body begins at the first "N.—(1)" form, so
# we drop everything between "ARRANGEMENT OF" and that first em-dash section.
_SG_BODY_START_RE = re.compile(r"(?m)^\s*\d{1,4}[A-Za-z]{0,2}\.\s*[‐-―]\s*\(\d")
...
def _strip_arrangement_toc(text: str) -> str:
    ...
    if not hdr:
        return text
    body = _SG_BODY_START_RE.search(text, hdr.end())
    if not body:
        return text

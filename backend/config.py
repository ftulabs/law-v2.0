...
class Settings(BaseSettings):
    ...
    # Whatever server a given machine actually uses — ours included — is configured
    # per-machine in `.env`, which is gitignored, and is named nowhere in this repository:
    #   LOCAL_LLM_BASE_URL / LOCAL_LLM_MODEL / LOCAL_LLM_API_KEY
    # This comment used to name our own host and model. That is a fact about one deployment,
    # it went stale twice in a month, and reading it as documentation of the system is how a
    # dead model name ends up being quoted back as what the pipeline calls.
    ...
    # Concurrent LLM grading calls, for a provider that cannot answer for itself. Fitted to
    # ONE CLOUD ENDPOINT: deepseek throughput plateaus ~12, so 16 is headroom without the
    # regression seen at 24+. That is a rate-limit ceiling, and it is the wrong shape of number
    # for a self-hosted cluster, whose capacity is its node count and which has no rate limit
    # at all — 16 threads across thirteen nodes leaves most of them idle. `LocalLLM` overrides
    ...
    # cluster starts returning HTTPError instead of answering. Tune via
    # LOCAL_LLM_CALLS_PER_NODE.
    local_llm_calls_per_node: int = 3
    # Grading calls to keep in flight at a vLLM node. vLLM batches continuously and will not
    # state a ceiling, so unlike llama.cpp's `total_slots` this cannot be read off the server —
    # it is measured. Against the team's own vLLM (Qwen3.6-35B-A3B-AWQ) with the run's real
    # 4,051-token prompt, 2026-09-18:
    #
    #     threads    2      4      8     16     32     64
    #     calls/s  0.80   1.33   2.36   4.02   5.03   4.09
    #
    # Throughput climbs to 32 and turns over by 64: past the peak the scheduler holds more
    # requests than it can keep resident and adds queueing time for nothing. Tune via
    # LOCAL_LLM_VLLM_CONCURRENCY.
    local_llm_vllm_concurrency: int = 32
    # Per-economy shortlist budget (data/retrieval_budget.json, generated by
    # tools/measure_budget.py). The numbers above are a SINGLE curve fitted to three economies
    # at once, and the per-economy split shows why that is a compromise: Singapore reaches
    # provision-recall 1.000 at k=40 while Australia needs 300. An economy with no measured
    # entry keeps the formula above unchanged, so switching this off can only ever cost money,
    # never coverage. Off → every economy uses the untuned default.
    retrieval_budget_enabled: bool = True
    # Circuit breaker for the grading loop. If this many calls have failed and NOT ONE has
    # succeeded, the run stops issuing calls: whatever is wrong is systemic (dead key, spend
    # cap, provider outage), not per-provision. Measured cost of not having it: a Singapore
    # pillar-6 run on an exhausted key made 968 calls, every one of them doomed, and reported
    # a spend cap as a bad key. A provider that classifies the failure as terminal
    # (LLMTerminalError) trips the breaker on the FIRST call, without waiting for this count.
    mapping_failure_breaker: int = 25
